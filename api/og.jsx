...
export default async function handler(req) {
  ...
  const ids = decodeIds(url.searchParams.get('ids'));
    ...
    (
      <div
        style={{
          ...
          background: '#ffffff', color: '#0f172a', padding: 48, fontSize: 36,
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <div style={{
            width: 18, height: 18, background: '#00419b',
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginTop: 24 }}>
          ...
          <div style={{ fontSize: 64, fontWeight: 800, color: '#00419b' }}>
            {Number.isFinite(count) ? count : 0}件
          </div>
        </div>
      </div>
    ),
